...
**React↔D3 boundary:** `components/map/MapCanvas.tsx` is the sole boundary. React mounts the paths; D3 owns the `<g>` zoom transform, both driven programmatically (route changes → `fitTransform` transitions, see `components/map/ZoomController.ts`) and bound live to the `<svg>` for interactive mouse-drag/wheel/touch pinch-pan. `zoom.clickDistance` (and matching `scaleExtent`/`translateExtent`/`extent` set in the same viewBox-unit space `lib/geo.ts` uses) keeps a stationary tap passing through to each region's `<Link>` instead of being swallowed as a drag; manual pan/zoom is ephemeral UI state, never written to the URL. Only the active province's cantons and the active canton's districts are mounted, never all 494 districts. `GeoLayer` is written once and reused for all three layers. Map animations (intro border draw, staggered child reveals) are CSS keyframes in `app/globals.css`, applied as layer classes; they use `backwards` fill so class-driven hover/dim states take over when they end.
...
- Tailwind CSS 4 via `@tailwindcss/postcss` (see `postcss.config.mjs`); `app/globals.css` holds the `@import "tailwindcss"`, the "Pura Vida" palette tokens, and the map animation keyframes — no custom CSS files elsewhere.
- **Colors go through semantic tokens**, never raw gray/`dark:` utilities: `bg-lienzo` (page), `bg-superficie` (cards), `border-borde`, `text-tinta`, `text-suave` (muted), `bg-acento`/`text-acento-tinta`. They flip with `prefers-color-scheme` via CSS variables in `globals.css` (`@theme inline`), so components need no `dark:` variants for chrome colors. Province fills are the exception: Tailwind palette classes, light 300s + saturated `dark:` 400s, full opacity, keyed by province `codigo`, defined in `lib/provinceColors.ts` (Guanacaste alone uses the deeper 500/600 shade — see that file's comment — since 300/400 gold was indistinguishable from Heredia's amber).
